...
Ce site est le support de présentation final du rapport de la SAÉ S1.04. Il synthétise l'étude de cas KDou, en détaillant :
    ...
    L'analyse de la qualité des données et de la modélisation (clés primaires/étrangères, anomalies).

    Les résultats des requêtes SQL clés pour l'exploration de données (localisation, catégories, fournisseurs, commandes).
...
Ce projet respecte les contraintes académiques de la ressource R1.02, assurant un contrôle total sur la structure et le style.
    ...
    Outil BDD : MS Access (ou équivalent) pour la manipulation initiale de la base de données.
...
Ce projet étant un site statique, il peut être consulté directement :
    ...
    Ouvrir le fichier index.html dans n'importe quel navigateur web moderne (Chrome, Firefox, Edge, etc.).
